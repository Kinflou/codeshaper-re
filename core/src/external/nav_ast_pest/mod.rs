...
pub static NAVIGATORS: Lazy<HashMap<&str, fn() -> Box<dyn Navigator>>> = Lazy::new(||
    HashMap::from([
        ("small_config", SmallConfigNavigator::new_box as _)
    ])
);
...
impl PestNavProvider {
    ...
    pub fn new_box() -> Box<dyn NavigatorProvider> { Box::new(Self {}) }
}
