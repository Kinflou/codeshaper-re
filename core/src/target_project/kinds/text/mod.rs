    ...
    pub fn from_target(path: &'a Path) -> Result<Rc<RefCell<TextSolution>>> {
        ...
        fn group_recursive(
            ...
            for (name, project_group) in &group.groups {
                let children = group_recursive(name, project_group, Rc::clone(&solution_group));

                RefCell::borrow_mut(&solution_group)
                    .groups
                    .push(Rc::downgrade(&children) as _);
            }
        ...
        let root_group = group_recursive(&"root".to_string(), &project.groups["root"], root_group);
