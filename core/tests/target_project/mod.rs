...
fn find_kind_initializer() {
    let initializer = target_project::find_initializer("plain_text").unwrap();

    assert_eq!(
        *initializer as usize,
        text::TextSolution::from_path_shared as usize
    );
}
...
fn load_plain_text_target() {
    let target = target_project::from_kind_path(
        "plain_text",
        Path::new("/tests/data/test_workspace/target/origin/"),
    )
    .unwrap();
