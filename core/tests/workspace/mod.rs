...
static TEST_WORKSPACE_CONFIG_PATH: Lazy<String> =
    Lazy::new(|| format!("{}/config.json5", TEST_WORKSPACE_DIR));

#[test]
fn load_workspace_config() {
    let config = WorkspaceConfig::from_file(Path::new(&*TEST_WORKSPACE_CONFIG_PATH)).unwrap();
